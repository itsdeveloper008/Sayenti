...
export function ServicesSpotlightCarousel() {
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel({
    ...
    skipSnaps: false,
    containScroll: false,
    // Embla "duration" is a tween factor, not ms - higher is slower/smoother.
    duration: reduce ? 20 : 45,
  });
  ...
  const onSelect = useCallback(() => {
    ...
    // Restart the progress bar whenever the slide changes (auto or manual).
    setCycle((c) => c + 1);
  }, [emblaApi]);
  ...
  return (
    ...
    >
            ...
            {bentoServices.map((service, i) => {
              ...
              return (
                ...
                >
                  <motion.article
                    className={cn(
                      "relative flex h-full min-h-[380px] flex-col overflow-hidden rounded-3xl border will-change-transform md:min-h-[440px]",
                      active
                        ? "border-white/15 shadow-[0_24px_80px_rgb(0_0_0_/_0.55)]"
                        : "border-white/5"
                    )}
                    animate={{
                      scale: reduce ? 1 : active ? 1 : 0.9,
      ...
      {/* Progress rail - each segment fills over the 3s autoplay window */}
      <div className="relative z-10 mt-10 flex items-center justify-center gap-4 md:mt-12">
        <button
          type="button"
          onClick={() => emblaApi?.scrollPrev()}
          ...
          {bentoServices.map((s, i) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "relative h-1 overflow-hidden rounded-full bg-white/20 transition-[width] duration-500 ease-out",
                  isActive ? "w-12" : "w-3 hover:bg-white/40"
                )}
              >
                {isActive && (
                  <motion.span
                    ...
                    animate={{ width: "100%" }}
                    transition={{
                      duration: playing ? AUTOPLAY_MS / 1000 : 0.2,
                      ease: "linear",
                    }}
                  />
                )}
